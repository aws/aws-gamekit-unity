...
namespace AWS.GameKit.Runtime.Features.GameKitGameSaving
{
    ...
    /// [Initialization]
    /// <para>
    /// The Game Saving library will be automatically initialized, optionally the SetFileActions() can be called immediately after initialization. This initialization must be done before calling any other Game Saving APIs, and should
    /// only be done once in the lifetime of your program because the Game Saving library internally uses a singleton.
    ///
    /// After the library is initialized, each time a user logs in, GameKitClearSyncedSlots(), GameKitAddLocalSlots(), and GameKitGetAllSlotSyncStatuses() must be called in that order to ensure all local and cloud slots are up to date.
    ...
    /// prevent malicious players from storing too much data in the cloud. You can change this limit by doing another deployment through the AWS GameKit Settings UI.
    /// </para>
    /// 
    /// [Save Information]
    /// <para>
    /// The local and cloud attributes for a save slot are collectively known as "save information" and are stored in the Slot class.
    /// </para>
    /// 
    /// [Cached Slots]
    /// <para>
    ...
    /// </para>
    /// </remarks>
    public interface IGameSavingProvider
    {
        /// <summary>
